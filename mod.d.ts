...
/**
* Interface describing `acoversinBy`.
*/
interface Routine {
	/**
	...
	*/
	...
	/**
	* Computes the inverse coversed sine of each element retrieved from an input strided array `x` via a callback function and assigns each result to an element in an output strided array `y` using alternative indexing semantics.
	*
	* @param N - number of indexed elements
	* @param x - input array
	* @param strideX - `x` stride length
...
* // y => [ ~1.571, ~-0.607, ~0.524, 0.0, ~-0.253 ]
*/
declare var acoversinBy: Routine;
